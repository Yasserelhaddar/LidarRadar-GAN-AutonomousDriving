...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        feature = F.relu(self.bn2(self.conv2(x)))
        x = F.relu(self.bn3(self.conv3(feature)))
        x = self.residual(x)
        ...
        return torch.max(x, 1, keepdim=True)[0], feature  # [bs, 1, 512]


class Decoder(nn.Module):
    def __init__(self, num_points=8192, m=8100):
        super(Decoder, self).__init__()
        self.n = num_points  # input point cloud size.
        self.m = m  # 45 * 45.
        self.meshgrid = [[-0.3, 0.3, 90], [-0.3, 0.3, 90]]
    ...
    def forward(self, input, feature):
        ...
        #concate3 = torch.cat((concate2, feature), dim=1)  # [bs, 643, m]
        after_folding2 = self.mlp2(concate2)  # [bs, 3, m]
        return after_folding2.transpose(1, 2), concate2  # [bs, m ,3]
...
class FoldNet(nn.Module):
    ...
    def forward(self, input):
        codeword, feature = self.encoder(input)
        print(feature.shape)
        output, concate2 = self.decoder(codeword, feature)
