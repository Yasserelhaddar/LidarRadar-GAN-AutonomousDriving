...
def distChamfer(a, b):
    ...
    distance = torch.min(P, 2)[0] + torch.min(P, 1)[0]
...
points1 = torch.rand(1, 8000, 3).cuda()
points2 = torch.rand(1, 8000, 3, requires_grad=True).cuda()

print("dist: ", distChamfer(points1, points2).cuda())
print("dist: ", distChamfer(points2, points1).cuda())'''
